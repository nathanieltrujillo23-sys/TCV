...
export function TransactionCountChart({ income, expense }: { income: TrendPoint[]; expense: TrendPoint[] }) {
  ...
  return (
    <div className="relative overflow-x-auto -mx-1 px-1">
      <svg width={width} height={chartHeight + 18} role="img" aria-label="Transaction count per period">
        {income.map((p, i) => {
        ...
        <ChartTooltip x={hover * slot + (slot - groupWidth) / 2} y={chartHeight / 2}>
          <div className="flex flex-col gap-0.5">
            ...
            <span className="text-emerald-400">
              {income[hover].value} income {income[hover].value === 1 ? "entry" : "entries"}
            </span>
            <span className="text-rose-400">
              {expense[hover]?.value ?? 0} expense {(expense[hover]?.value ?? 0) === 1 ? "entry" : "entries"}
            </span>
          </div>
        </ChartTooltip>
      )}
